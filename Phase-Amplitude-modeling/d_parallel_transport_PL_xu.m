...
if (phi <= pi/2)
 ...
 % with respect to 'x' along the direction 'uu'
 x12_xu = -xi*sin(s*phi)*s*dphi_xu +...% the second term
     cross(dnormal_xu, xi)*sin(s*phi) +...
     cross(normal, xi)*cos(s*phi)*s*dphi_xu +...% the third term
     dnormal_xu*dot(normal, xi)*(1 - cos(s*phi)) +...
     normal*dot(dnormal_xu, xi)*(1 - cos(s*phi)) +...
     normal*dot(normal, xi)*sin(s*phi)*s*dphi_xu;
 % parallel transport of 'qi' from 'xi' to 'x12' along 
 ...
 % along the direction 'uu'
 xx12_xu = (x12_xu)/norm(xi + x12) -...
     (xi + x12)*dot(xi + x12, x12_xu)/(norm(xi + x12)^3);
 % the anlge 'theta11' defining the 'beta11'
 theta11 = asin(dot(xx12, normal));
 % the derivative of 'theta11' with respect to 'x'
 % along the direction 'uu'
 theta11_xu = (dot(xx12_xu, normal) +...
     dot(xx12, dnormal_xu))/(1 - dot(xx12, normal)^2)^(1/2);
 ...
 % along the direction 'uu'
 parq11_xu = d_parallel_transport_PL_x222(qi, xi, x12, theta11, x12_xu) +...
     d_parallel_transport_PL_theta2(qi, xi, x12, theta11)*theta11_xu;
 % parallel transport of 'parq11' from 'x12' to 'x' along 
 % the 'beta22' which is the second half of 'beta'
 % defined by angle between 'normal' and 'xx12' 
 xx12 = (x12 + x)/norm(x12 + x);
 % the derivative of 'xx12' with respect to 'x'
 % along the direction 'uu'
 xx12_xu = (x12_xu)/norm(x12 + x) -...
     (x12 + x)*dot(x12 + x, x12_xu)/(norm(x12 + x)^3);
 % the anlge 'theta22' defining the 'beta22'
 theta22 = asin(dot(xx12, normal));
 % the derivative of 'theta22' with respect to 'x'
 % along the direction 'uu'
 theta22_xu = (dot(xx12_xu, normal) +...
     dot(xx12, dnormal_xu))/(1 - dot(xx12, normal)^2)^(1/2);
 % the final parallel transport of 'parq11' from 'x12' to 'x' 
 % parqi = parallel_transport_PL2(parq11, x12, x, theta22);
 % the derivative of 'parqi' with respect to 'x' 
 % along the direction 'uu'
 dparqi_xu = parallel_transport_PL2(parq11_xu, x12, x, theta22) +...
     d_parallel_transport_PL_x122(parq11, x12, x, theta22, x12_xu) +...
     d_parallel_transport_PL_x222(parq11, x12, x, theta22, uu) +...
     d_parallel_transport_PL_theta2(parq11, x12, x, theta22)*theta22_xu;
end
end
