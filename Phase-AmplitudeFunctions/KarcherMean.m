function [mup,muq,mupath, KF]= KarcherMean(patharray,str, NN, MM)
...
%       -- NN is the number of points on the baseline
%       -- MM is the number of points on the range of theta, among which
%                 one chooses the optimal value of the function
...
%       -- KF : Karcher Function, achieved optimal objective value     
N = length(patharray);
...
while (iter<maxiter)
    ...
    for i=1:N
        if(str =='fast')
            ...
            [epmu(:,i),epmw(:,:,i),dsq]=InverseExponentialMap(mupath,patharray{i}, NN, MM);        
        end;
        disp(dsq);
        tmpsumd = tmpsumd + dsq;
    end;
    ...
    sumd(iter) = tmpsumd; KF = tmpsumd/N;
